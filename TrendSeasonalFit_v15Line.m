function rec_cg = TrendSeasonalFit_v15Line(dir_l,n_rst,ncols,nrows,T_cg,Tmax_cg,conse,num_c,nbands,B_detect)
...
line_t = zeros(num_t,(nbands-1)*ncols); %Ys
...
for i=1:num_t
    ...
    fseek(fid_t,num_byte*(nrows-1)*ncols*(nbands-1),'bof');
    line_t(i,:) = fread(fid_t,(nbands-1)*ncols,'int16=>double','ieee-le'); % get Ys
end
fclose('all'); % close all files

for i_ids = 1:ncols
    % mask data
    line_m = line_t(:,(nbands-1)*i_ids);
    ...
    line_t(:,(nbands-1)*(i_ids-1)+7) = line_t(:,(nbands-1)*(i_ids-1)+7)*10 - 27315;
    
    % clear pixel should have reflectance between 0 and 1
    % brightness temperature should between -93.2 to 70.7 celsius degree
    idrange = line_t(:,(nbands-1)*(i_ids-1)+1)>0&line_t(:,(nbands-1)*(i_ids-1)+1)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+2)>0&line_t(:,(nbands-1)*(i_ids-1)+2)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+3)>0&line_t(:,(nbands-1)*(i_ids-1)+3)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+4)>0&line_t(:,(nbands-1)*(i_ids-1)+4)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+5)>0&line_t(:,(nbands-1)*(i_ids-1)+5)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+6)>0&line_t(:,(nbands-1)*(i_ids-1)+6)<10000&...
        line_t(:,(nbands-1)*(i_ids-1)+7)>-9320&line_t(:,(nbands-1)*(i_ids-1)+7)<7070;
    ...
    if clr_pct < t_clr
        ...
        if sn_pct > t_sn
            ...
            if n_sn < n_times*min_num_c % not enough snow pixels
                ...
                % bands 1-5,7,6
                clry(:,1:nbands-2) = line_t(idgood,((nbands-1)*(i_ids-1)+1):((nbands-1)*(i_ids-1)+nbands-2));
                % NDVI

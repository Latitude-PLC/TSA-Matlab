function rec_cg = TrendSeasonalFit_v13_1Plot(N_row,N_col,T_cg,conse,B_detect)
% function rec_cg = TrendSeasonalFit_v12Plot(3845,2918,0.99,6,2:6);
...
%
%% Revisions: $ Date: 11/27/2015 $ Copyright: Zhe Zhu
...
for i=1:num_t
    ...
    % read ref+thermal+Fmask
    dummy_name = im;
    fid_t = fopen(dummy_name,'r'); % get file ids
    fseek(fid_t,num_byte*((N_row-1)*dim(2)+N_col-1)*(nbands),'bof');
    line_t(i,1:nbands) = fread(fid_t,nbands,'int16=>double','ieee-le'); % get Ys
    
    % read std345
    dummy_name = imstd;
    fid_t = fopen(dummy_name,'r'); % get file ids
    fseek(fid_t,num_byte*((N_row-1)*dim(2)+N_col-1)*3,'bof');
    line_s(i,:) = fread(fid_t,3,'uint16=>double','ieee-le'); % get Ys
...
if clr_pct < t_clr
    ...
    if sn_pct > t_sn
        ...
        if n_sn < n_times*min_num_c % not enough snow pixels
            ...
            clrx = sdate(idgood);
            % bands 1-5,7,6
            clry = line_t(idgood,1:nbands);
            % clry(:,end) = n_lst(idgood);
            
            % bands 1-5,7,6
            clry_spe = line_t(idgood,1:nbands-1);
            % standard deviation
            clry_std = line_s(idgood,:);
            % spectral + stdandard deviation
            clry = [clry_spe,clry_std];
            ...
            % record texture information
            rec_cg(num_fc).texture = median(clry(:,nbands:end),1);
        end
        ...
        clrx = sdate(idgood);
        % bands 1-5,7,6
        % clry = line_t(idgood,[1:nbands-1,nbands+1:nbands+3,nbands]);
        % clry(:,end) = n_lst(idgood);
        ...
        if i_span >= n_times*min_num_c && time_span >= mini_yrs
                    ...
                    if time_span < mini_yrs
                        ...
                        if v_dif > T_cg
                            ...
                            if num_fc == rec_fc ...
                                ...
                                % record texture information
                                rec_cg(num_fc).texture = median(clry(i_break:i_start-1,nbands:end),1);
            ...
            if BL_train == 1
                ...
                if  i_count == 0  || i_span <= max_num_c*n_times
                    ...
                    rec_cg(num_fc).magnitude = zeros(1,nbands-1);
                    % record texture information
                    rec_cg(num_fc).texture = median(clry(IDs,nbands:end),1);
    ...
    elseif BL_train == 0
        ...
        if length(clrx(i_start:end)) >= conse
            ...
            rec_cg(num_fc).magnitude = zeros(1,nbands-1);
            % record texture information
            rec_cg(num_fc).texture = median(clry(i_start:end,nbands:end),1);
        end
    end
end
...
% figure
% set(gcf,'Position',[0 300*nbands-1 3000 300]);
% set(gca,'FontSize',13)
% plot(clrx,clry,'.')
...
for B_plot = B_detect
    ...
    elseif B_num == 8
        ylabel('Band 3 Std (X10^4)');
    elseif B_num == 9
        ylabel('Band 4 Std (X10^4)');
    elseif B_num == 10
        ylabel('Band 5 Std (X10^4)');
    else
...
pause
close all
fprintf('CCDC v12.31 Testing Plots!\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%end of plot
% end of function
end
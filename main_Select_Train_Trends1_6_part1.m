...
%
% Version 1.6  Classify value from current year (04/06/2016)
% Version 1.5  Add new disturbed Land Cover Trend class (04/05/2016)
...
function main_Select_Train_Trends1_6_part1
...
im_roi(im_roi==3|im_roi==4) = 10;
% put mining into disturbed class (10)
...
% cd CCDCMap
% % Find new classes before the reference year
% im_change = enviread('ChangeMap'); % Change Map
% im_new = zeros(nrows,ncols);
% for i = 1:fyear
%     im_new(im_change(:,:,i)>0)=1;
% end
% clear im_change;
% cd ..
...
for i=1:rec_l
    ...
    for j = 1:length(ids_line)
        ...
        if (t_start(id_ref) < gt_start && t_end(id_ref) > gt_end) || ...
                (tim_roi(pos_ref) == 10 && t_start(id_ref) < gt_end & t_end(id_ref) > gt_end) % & floor(categ(id_ref)/10)~=3 
            ...
            tmp_cft(1,:) = tmp_cft(1,:)+gt_end*tmp_cft(2,:);
            ...
            % Y(plusid,2) = tim_new(pos_ref); % new (1) or old (0)
        end
    end
    i_row = i_ids(i);        
end

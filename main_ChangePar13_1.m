function main_ChangePar13_1(task,ntasks)
...
[jiDim,~,~,zc] = envihdrread([imf(1,:),'/',filename.name]);
...
if task == 1
    fileID = fopen('CCDC_Change_log.txt','w');
    ...
    % updates
    fprintf(fileID,'******************************************************************************************************\r\n');
    fprintf(fileID,'Revisions: $ Date: 02/25/2016 $ Copyright: Zhe Zhu\r\n');
    fprintf(fileID,'Version 13.01  Fixed a bug in t_start definition at model initialization (02/25/2016)\r\n');
    fprintf(fileID,'Version 13.00  Add Bands 3-5 texture for classification (02/25/2016)\r\n');
...
for i = 1:length(irows)
    ...
    try
        ...
        load([dir_l,'/',name_rst,'/','record_change',sprintf('%d',irows(i)),'.mat']);
    catch me
        ...
        TrendSeasonalFit_v13_1Line(dir_l,name_rst,ncols,irows(i),T_cg,Tmax_cg,conse,max_c,nbands,B_detect);
    end
end

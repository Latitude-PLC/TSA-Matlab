function rec_cg = TrendSeasonalFit_v15Plot(N_row,N_col,T_cg,conse,B_detect)
% function rec_cg = TrendSeasonalFit_v15Plot(3845,2918,0.99,6,1:9);
% CCDC 15.01 version - Zhe Zhu, EROS, USGS
...
%  Version 15.01  Fixed a bug for pixels without minimum observations (11/20/2015)
%% Version 15.0   Add NDVI, NBR, blue, and TIR bands as change indicators (10/01/2015)
%% Version 14.0   A change is defined based on maximum change in all bands (10/01/2015)
%  Version 13.10  Modified fit for perennial snow and Fmask failed pixels (09/20/2015)
%  Version 13.09  Do not fit with data less than 12 obs and 1 year (09/18/2015)
%  Version 13.08  Fixed a bug for missing values in land cover maps (09/16/2015)
%  Version 13.07  Fixed bugs for persistent snow and falied Fmask pixels (06/17/2015)
%  Version 13.06  Connected time for all models (05/28/2015)
%  Version 13.05  Bug fixed in snow percent (05/19/2015)
...
% cateogry category 3x: NA                 2x: end fit
...
inbands = 8;
% total ouput bands (1-5,7,6,NDVI,& NBR)
nbands = 9;
...
filename = dir([imf(1,:),'/','L*stack']); %%% Revision v12.16
...
line_t = zeros(num_t,nbands-1); %Ys
n_lst = zeros(num_t,1); % sensor number
...
for i=1:num_t
    ...
    fseek(fid_t,num_byte*((N_row-1)*dim(2)+N_col-1)*(nbands-1),'bof');
    line_t(i,:) = fread(fid_t,nbands-1,'int16=>double','ieee-le'); % get Ys
    n_lst(i) = str2num(imf(i,3));
end
...
if clr_pct < t_clr
    ...
    if sn_pct > t_sn
        ...
        if n_sn < n_times*min_num_c % not enough snow pixels
            fprintf('Not enough "good" observations!\n');
            return;
        else
            % start model fit for snow persistent pixels
            fprintf('Fit snow and clear observations (%.0f%%)!\n',100*sn_pct);
            
            % Xs & Ys for computation
            clrx = sdate(idgood);
            % intialize clry
            clry = zeros(length(clrx),nbands);
            % bands 1-5,7,6
            clry(:,1:nbands-2) = line_t(idgood,1:nbands-2);
            % NDVI
            clry(:,nbands-1) = 10000*(clry(:,4)-clry(:,3))./(clry(:,4)+clry(:,3)+0.01);
            % NBR
            clry(:,nbands) = 10000*(clry(:,4)-clry(:,6))./(clry(:,4)+clry(:,6)+0.01);
            % clry(:,end) = n_lst(idgood);
            ...
            for i_B = 1:nbands
                if i_B ~= nbands-2 % treat saturated and unsaturated pixels differently
                    idgood = clry(:,i_B) < 10000; % saturate if ref > 1
                    i_span = sum(idgood);
        ...
        if n_clr < n_times*min_num_c % not enough clear pixels
            ...
            % start model fit for Fmask failed pixels
            fprintf('Fmask failed (%.0f%%)!\n',100*clr_pct);
            
            % Xs & Ys for computation
            clrx = clrx(idclr);
            clry = clry(idclr,:);
            clry(:,end) = n_lst(idclr);
    ...
    var_clry = clry(2:end-1,:)-clry(1:end-2,:);
                                    ...
                                    for i_conse = 1:ini_conse
                                        for i_B = 1:nbands
                                            ...
                                            if sum(i_B == B_detect)
                                                ...
                                                % z-scores
                                                v_dif(i_conse,i_B) = abs(v_dif_mag(i_conse,i_B))/mini_rmse;
                                            end
                                        end
                                    end
                                    
                                    % change detection
                                    if max(min(v_dif(:,B_detect),[],1)) > T_cg % change detected
                                        break
                                    elseif max(v_dif(1,B_detect)) > Tmax_cg % false change
                                        % remove noise
                                        clrx(i_ini) = [];
                                        clry(i_ini,:) = [];
                ...
                if  i_count == 0  || i_span <= max_num_c*n_times
                    ...
                    for i_B = 1:nbands
                        ...
                        if sum(i_B == B_detect)
                            ...
                            % z-scores
                            v_dif(conse,i_B) = abs(v_dif_mag(conse,i_B))/mini_rmse;
                        end
                    end
                end
                
                [max_min_v,max_min_b] = max(min(v_dif(:,B_detect),[],1));
                % change detection
                if max_min_v > T_cg % change detected
                    fprintf('Change Magnitude = %.2f in %d Band\n',max_min_v, max_min_b);
                    time = datevec(clrx(i+1));
                    [time(1),clrx(i+1)-datenum(time(1),1,0)] %#ok<NOPRT>
                    % record break time
    ...
    if BL_train == 1
        ...
        for i_conse = conse:-1:1
            if max(min(v_dif(i_conse:conse,B_detect),[],1)) <= T_cg
                % the last stable id
                id_last = i_conse;
                break;
            end
...
for B_plot = B_detect
    ...
    for i=1:num_fit
        ...
        elseif rec_cg(i).category > 30 && rec_cg(i).category < 40
            plot(x_plot,pred_y,'-.','color',color_tab(icolor,:));
        else
            plot(x_plot,pred_y,'k-.','LineWidth',0.5);
        end
        
        hold on;
        if rec_cg(i).change_prob > 0
            if rec_cg(i).change_prob < 1
                plot(rec_cg(i).t_break,Ys(Xs==rec_cg(i).t_break,B_plot),'ko','Markersize',10);
            else
...
pause;
close all;
fprintf('CCDC v15.01 Testing Plots!\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%end of plot
% end of function
end
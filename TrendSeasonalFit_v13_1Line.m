function rec_cg = TrendSeasonalFit_v13_1Line(dir_l,n_rst,ncols,nrows,T_cg,Tmax_cg,conse,num_c,nbands,B_detect)
% CCDC 13.00 version - Zhe Zhu, EROS USGS
...
%
%% Revisions: $ Date: 02/25/2016 $ Copyright: Zhe Zhu
%  Version 13.01  Fixed a bug in t_start definition at model initialization (02/25/2016)
%  Version 13.00  Add Bands 3-5 texture for classification (02/25/2016)
...
rec_cg = struct('t_start',[],'t_end',[],'t_break',[],'coefs',[],'rmse',[],...
    'pos',[],'change_prob',[],'num_obs',[],'category',[],'magnitude',[],'texture',[]);
...
line_s = zeros(num_t,3*ncols); % texture
...
for i=1:num_t
    ...
    dummy_name = imstd;
    fid_t = fopen(dummy_name,'r'); % get file ids
    fseek(fid_t,num_byte*(nrows-1)*ncols*3,'bof');
    line_s(i,:) = fread(fid_t,3*ncols,'uint16=>double','ieee-le'); % get Ys
end
    ...
    if clr_pct < t_clr
        ...
        if sn_pct > t_sn
            ...
            if n_sn < n_times*min_num_c % not enough snow pixels
                ...
                % bands 1-5,7,6
                clry_spe = line_t(idgood,((nbands)*(i_ids-1)+1):((nbands)*(i_ids-1)+(nbands)-1));
                % standard deviation
                clry_std = line_s(idgood,(3*(i_ids-1)+1):(3*(i_ids-1)+3));
